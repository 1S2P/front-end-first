-- ─── 005: Workflow reassignment semantics ─────────────────────────────────────
-- 1. When a workflow task is sent back for changes (request_changes), the
--    previously completed sibling tasks of the same workflow instance must no
--    longer be marked completed — they are reverted to 'approved' and the
--    instance goes back to 'running'.
-- 2. Tasks stay 'completed' only when the whole workflow finishes: the
--    advance_workflow completion branch now marks every task of the instance
--    as completed.

-- ─── 1. advance_workflow: guard recursion + complete all tasks on finish ──────
...
-- ─── 2. review_task: request_changes un-completes sibling tasks ────────────────
